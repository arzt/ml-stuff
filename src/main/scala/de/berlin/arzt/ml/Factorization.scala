...
object Factorization {
  ...
  def gradientFlat(y: Matrix, unrated: DenseMatrix[Boolean], λ: Double)(x: Vec): Vec = {
    val (α, β) = fromFlat(y.rows, y.cols, x)
    val (dα, dβ) = gradient(y, unrated, λ)(α, β)
    toFlat(dα, dβ)
  }

  def gradient(y: Matrix, unrated: DenseMatrix[Boolean], λ: Double)(α: Matrix, β: Matrix) = {
    val μ = α.t * β
    val δ = μ - y
    δ(unrated) := 0.0
    val dα = β * δ.t + λ * α
  ...
  def trainModel(y: Matrix, rated: DenseMatrix[Boolean], unrated: DenseMatrix[Boolean], rank: Int, ε: Double, λ: Double) = {
    ...
    val x = toFlat(α, β)
    gradientDescent4(0.001, ε)(loss(y, rated, λ), gradientFlat(y, unrated, λ))(x)
  ...
  def main(args: Array[String]) {
    ...
    val result1 = gradientDescent4(a, ε, adjust = 10)(loss(y, rated, λ), gradientFlat(y, unrated, λ))(x1)

    val (rowr, colr) = fromFlat(n, m, x1)

    val l1 = lossUnflattened(y, rated, λ)(rowr, colr)
    val l2 = lossUnflattened(y, rated, λ)(a1, b1)
    println(l1)
    println(l2)
  }
}